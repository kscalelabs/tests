    ...
    try:
        ...
        while time.time() - start_time < duration:
            ...
            if cycle_position < 0.25:  # Ramp up to positive
                ...
                velocity = 4 * amplitude * frequency if use_velocity else None
        ...
        print("\nTest interrupted in run_triangle_test!")
    finally:
...
async def main() -> None:
    ...
    parser.add_argument(
        "--amplitude", type=float, default=20.0, help="Triangle wave amplitude in degrees (default: 20.0)"
    )
    parser.add_argument("--frequency", type=float, default=0.5, help="Triangle wave frequency in Hz (default: 0.5)")
    ...
    try:
        async with pykos.KOS() as kos:
            collected_data = await run_triangle_test(
